...
class ConfidenceScorer:
    ...
    def score(self, claim: "Claim") -> ConfidenceResult:
        ...
        n = len(all_evidence)
        if n == 0:
            return ConfidenceResult.from_score(0.0, [
                ConfidenceFactor(
                    name="source_count",
                    raw_value="0 sources",
                    contribution=0.0,
        ...
        factors.append(ConfidenceFactor(
            ...
            contribution=round(diversity_score * _FACTOR_WEIGHTS["source_diversity"], 4),
            direction="POSITIVE",
        ...
        factors.append(ConfidenceFactor(
            ...
            + ("directly stated in source" if directness_score >= 1.0
               else "inferred from related evidence" if directness_score >= 0.5
        ...
        factors.append(ConfidenceFactor(
            ...
            explanation="Most recent evidence is " + (
                "current" if recency_score >= 0.8
                else "recent" if recency_score >= 0.5
        ...
        factors.append(ConfidenceFactor(
            name="doc_quality",
            raw_value=f"best: {all_evidence[qualities.index(best_quality)].source_type.value}" if all_evidence else "none",
            contribution=round(best_quality * _FACTOR_WEIGHTS["doc_quality"], 4),
            direction="POSITIVE" if best_quality >= 0.7 else "NEGATIVE",
            explanation=f"Highest-quality source is {all_evidence[qualities.index(best_quality)].source_type.value}" if all_evidence else "No sources",
        ))
    ...
    def _compute_recency(self, evidence: list) -> float:
        ...
        decay = 0.5 ** ((age_years - _RECENCY_FULL_CREDIT_YEARS) / _RECENCY_HALF_LIFE_YEARS)
