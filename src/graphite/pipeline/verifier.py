...
class ClaimVerifier:
    ...
    def __init__(self, api_key: Optional[str] = None, base_url: Optional[str] = None):
        try:
            ...
            raise ImportError("Please install the default verifier: pip install \"graphite-engine[llm]\"")
    ...
    def verify_claims(
        ...
        for claim in claims:
            ...
            supp_ids = [chunks[i]["document_id"] for i in res.get("supporting_evidence_indices", []) if i < len(chunks)]
            conf_ids = [chunks[i]["document_id"] for i in res.get("conflicting_evidence_indices", []) if i < len(chunks)]
...
def verify_claims(claims: List[Claim], evidence_map: Dict[str, List[Dict[str, Any]]], model: str = "gemini-2.5-flash", api_key: Optional[str] = None) -> List[Verdict]:
