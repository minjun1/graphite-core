...
from graphite.claim import Claim, Verdict, ArgumentVerdict, ArgumentVerdictEnum, VerdictRationale
...
class ArgumentAnalyzer:
    ...
    def __init__(self, api_key: Optional[str] = None, base_url: Optional[str] = None):
        try:
            ...
            raise ImportError("Please install the default analyzer: pip install \"graphite-engine[llm]\"")
    ...
    def analyze_argument_chain(
        ...
        for item in res_data.get("argument_verdicts", []):
            ...
            results.append(ArgumentVerdict(
                text=item.get("text", ""),
                verdict=v_enum,
                rationale=rationale,
...
def analyze_argument_chain(memo_text: str, verdicts: List[Verdict], model: str = "gemini-2.5-flash", api_key: Optional[str] = None) -> List[ArgumentVerdict]:
