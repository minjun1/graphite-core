...
from graphite.scenarios.weathernext_forecast import (
    forecast_to_scenario_shocks,
    compute_hazard_intensity,
)
...
def main():
    ...
    for node_id, fc in forecasts.items():
        ...
        print(
            f"   {intensity:.0%} | {wind_mph:5.0f}mph | {precip_in:4.1f}in | {node_id}"
        )
    print()
    ...
    if args.alphaearth:
        try:
            ...
            if os.path.exists(geom_path):
                ...
                print(
                    f"   🛰️  AlphaEarth: {stats['enriched']}/{stats['total']} nodes enriched"
                )

                # Level 2: inject edge similarity + modulate alpha
                n_ann = inject_edge_similarity(G)
                print(
                    f"   📐 Edge similarity: {n_ann}/{G.number_of_edges()} edges annotated"
                )
    ...
    if embedding_aware:
        ...
        effective_alpha = make_embedding_aware_alpha(
            G,
            base_alpha_fn,
            embedding_weight=0.4,  # same as Harvey
        )
